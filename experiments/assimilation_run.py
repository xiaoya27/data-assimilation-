...
import numpy as np
import time
import assimilation_conf_PEParametrizationOptimization as conf         #Load the experiment configuration
...
for ic in range(0,NCoef)  :
   if GeneralConf['RunPlotParameters'] & (DAConf['InitialPSigma'][ic] > 0)  :
      ...
      plt.savefig( GeneralConf['FigPath'] + '/' + GeneralConf['ExpName'] + '/RMSEP_LETKF_run.png', facecolor='w', format='png' )
      plt.show()
      #plt.close()
      ...
      plt.colorbar()
      plt.savefig( GeneralConf['FigPath'] + '/' + GeneralConf['ExpName'] + '/PEstimated' + CString + '.png', facecolor='w', format='png' )
      plt.show()
