...
GeneralConf['ExpName']='LETKF_PEParametrizationOptimization_d'            #Experiment name.
...
ModelConf['Coef']=np.array([19.0,0.0,0.0])             #Coefficient of parametrized forcing (polynom coefficients starting from coef[0]*x^0 + coef[1]*x ... ) 
...
DAConf['InfCoefs']=np.array([1.7,0.0,0.0,0.0,0.0])   #Mult inf, RTPS, RTPP, EPES, Additive inflation
...
DAConf['InitialPSigma']=np.array([0.035,0.035,0.0])  #Initial ensemble spread for the parameters. (0 means no parameter estimation)

DAConf['InfCoefsP']=np.array([1.2,0.0,0.0,0.0,0.0])  #Mult inf, RTPS, RTPP, EPES, Additive inflation
